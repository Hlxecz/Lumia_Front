...
export default function ProfileScreen() {
  ...
  const navigateToInfoScreen = (infoType: string) => {
    console.log(`MapsToInfoScreen called for: ${infoType}`);
    if (infoType === '오픈 라이선스') {
      router.push('/open-source-licenses');
      return;
    }
    if (infoType === '개인정보처리방침') {
    ...
    Alert.alert(infoType, '연결된 안내 화면을 찾을 수 없습니다.');
  };

  const navigateToInfoScreenLegacy = (infoType: string) => {
